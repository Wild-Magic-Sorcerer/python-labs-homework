...
Затем создайте класс `Bus`, наследующий `Transport`, который добавляет поле `route_number` и переопределяет метод `info()`, чтобы он также выводил номер маршрута.
...
Реализуйте методы `deposit(amount)` и `withdraw(amount)`, которые увеличивают и уменьшают баланс, не позволяя снимать сумму, превышающую остаток.
...
Создайте несколько объектов и продемонстрируйте:
   ...
   1. Работу цепочки наследования;
   2. Вызов родительских методов через `super()`;
   3. Полиморфизм: одинаковый вызов метода `introduce()` для разных классов.
